...
//-------------------------------------------------------------------
QCDJetsTemp::QCDJetsTemp():
iProgressBy(500000),
sHistFileName("QCDJetsTemp.root")
{
}

//--- do all my stuff here ------------------------------------------
void QCDJetsTemp::DoMyStuff(Stuple& stuple)
{
	...
	if (stuple.pho_Nloose < 1) return;
	
	std::vector<int> UsedPho, UsedEle;
	
	
	for (unsigned int i = 0; i < stuple.pho_num; ++i ) {
		if (stuple.pho_LooseId[i] != 0) continue;		// loose photons
...
//-------------------------------------------------------------
void QCDJetsTemp::Init(TChain *tree)
{
	iEventsSelected = 0;
	iPho1JetEvts = 0;
	iPho2JetEvts = 0;
...
//-------------------------------------------------------------------
void QCDJetsTemp::BookHistograms()
{
	...
	text2title1 << "#gamma^{sideband}+>=1Jets :";
	text2title2 << "#gamma^{sideband}+>=2Jets :";
...
//------ main -------------------------------------------------------
void QCDJetsTemp::Main(TChain *ch, int iRunEvents)
{
	if (ch) {
		myChain = ch;
		//ch->Print();	
