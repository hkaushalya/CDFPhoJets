/*{{{*/
/*  $Id: WSel.cc,v 1.1 2011/05/31 17:57:03 samantha Exp $
 *  $Log: WSel.cc,v $
 *  Revision 1.1  2011/05/31 17:57:03  samantha
 *  This is to get corrections to EWK MC by comparing W data and W MC.
 *
 */
...
void WSel::Init(TChain *tree)
{
	...
	if (UseNvtxWeights())
	{
		vNvtxWgts.push_back(0.703958);
		vNvtxWgts.push_back(1.07618);
		vNvtxWgts.push_back(1.5412);
		vNvtxWgts.push_back(2.21922);
		vNvtxWgts.push_back(3.76673);
		vNvtxWgts.push_back(8.43797);
		vNvtxWgts.push_back(24.0997);
		vNvtxWgts.push_back(75.8731);
		vNvtxWgts.push_back(297.555);
		vNvtxWgts.push_back(181.43);
...
//------ main -------------------------------------------------------
void WSel::Main(TChain *ch, int iRunEvents)
{
	TBenchmark timer;
	...
	std::cout << magenta << "======== WSel =======================" << std::endl;
	std::cout << "Entries found    = " << iENTRIES << std::endl;
...
void WSel::CreateElectronLists(const Stuple stuple, ElectronList& list, const int collection)
{
	/*{{{*/
	switch (collection)
	{
...
void WSel::GetCommonVariables(const Stuple stuple, CommonVars& cVars)
{
	/*{{{*/
...
void WSel::DoMyStuff(CommonVars cVars, ElectronList cEles)
{	
	...
	if (UsedEle.size()>=1)
	{
		...
		TLorentzVector tlEle1(cEles.ele_Px[iEle1Index], cEles.ele_Py[iEle1Index], cEles.ele_Pz[iEle1Index], cEles.ele_E[iEle1Index]);
		if (tlEle1.Pt()<20 || cVars.met_Met<20) return;
		if (UsedEle.size()>=2) ++iExtraEle; 
		used = JetSelAndHistFill(cVars, cEles, UsedEle, &Hmc, vMcCount);
	}

}

int WSel::JetSelAndHistFill(const CommonVars Vars, 
		...
		)
{
	...
	FillWHists(Vars, eles, &(hist->TwoEles), 0, fWeight);

	return 1;		//1=this event has been used!
}


void WSel::FillEventHists(const CommonVars& vars,
								const ElectronList& eles, 
...
void WSel::FillElectronHists(const CommonVars& cVars, const ElectronList& eles, Histograms::PhotonHists_t* hist,
...
void WSel::PrintHeader(const CommonVars& cVars) const
{
	std::cout << " ============== " << cVars.evt_RunNumber << ", " << cVars.evt_EventNumber << std::endl;
}

void WSel::PrintJobSettings()
{	
...
void WSel::FillWHists(const CommonVars& cVars, const ElectronList& eles, 
						...
						const float fWeight)
{
	...
	const TLorentzVector tlSum = tlEle + tlMet; 
