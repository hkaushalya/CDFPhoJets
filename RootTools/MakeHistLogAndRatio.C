 ...
 */
/*{{{*/
/*  $Id: MakeHistLogAndRatio.C,v 1.16 2012/10/29 19:41:08 samantha Exp $
 *  $Log: MakeHistLogAndRatio.C,v $
 *  Revision 1.16  2012/10/29 19:41:08  samantha
 *  Final commit.
 *
 *  Revision 1.15  2010/04/16 20:46:35  samantha
 *  MAJOR CHANGES: 1. I am modifying the code to do constant bin size histograms.
 *  Global variable bDO_VAR_BINNING allows to choose between constant and varibale
 *  size binning. Not complete yet.
 *  2. I am going trough this code with Dr.Dittmann.
 *
 *  BUG FIX: 1. CheckNegativeBin(): when checking for statictically negative bins, I
 *  had the paranthesis in the wrong place. Dr. Dittmann pointed this out.Fixed it.
 *  2. Also in the Reweighted sideband selection I was not clonning qcdjet and
 *  qcdjet_100 hists. Both pointers were pointing to the same object. I remember I
 *  was cloning them before. When I checked the revision history, this was a recent
 *  change during this new sample additions. I fixed it now the two pointers refer
 *  to two copies of the same object.
 *  3. Lum error calculation had a bug. My implementation was wrong.
 *
 *  ADDED: 1. WhichSample() to get the string corresponding name for each sample.
 *  2. jeteta_fitFunction() to fit jet eta weights.
 *  3. MakeJetEtaWeightsForSideband() to derive lead jet detector eta base weights.
 *  4. Added few more hists to the default list of hists.
 *
 *  MODIFIED: GetCosmicErr(): assigning const error of 1 for all non-zero bins in
 *  the cosmic template.
 *
 *  Revision 1.14  2010/03/18 16:28:36  samantha
 *  BUG FIX: 1. When studying the MET plots I noticed that the actual integral of
 *  backgrounds are not what they supposed to be. When I scale histogram to their
 *  expected number, I was using hist->GetIntegral() method which apparently does
 *  not consider the bin width. Since I do a variable sized binning, my total
 *  integral should take bin width into its calculation. So I fixed this by using
 *  hist->Integral("width") which multiplies bin content by bi width when
 *  calculating integral. This fixed the large discrepency in the MET plot. Other
 *  plots are not affected (rather not noticeable) as cosmic/halo/ewk backgrounds
 *  are very small.
 *  2. Also the EWK lum error is porperly calculated by varying the normalization
 *  scale by Lum+1sigma and taking the difference.
 *
 *  ADDED:  0. dipho mc contribution for the MET plot is complete. dipho mc is
 *  included on for the MET plot. It is insignificant in other plots.
 *  	1. Two more samples, iG30MET40 and iG30EXCL1J
 *  	2. CheckNegativeBin() to check for hists with statistically significant
 *  negative bin when backgrounds are subtracted from a hist (eg. EWK/HALO/COSMIC
 *  background are subtracted from qcd hist)
 *  	3.SubtractCosmicFromQCD() to subtract cosmic background from qcd hist.
 *  	4. SubtractEWKfromQCD() to subtract EWK component in qcd hist
 *  	5. Final hist y-axis min, max is dynamically detemined for each hist now
 *  	6. The binning has to adjusted for different samples. Especially for
 *  iG30MET40 which has limited stats. Code is modified so the default binning can
 *   be overridden for a given sample.
 *
 *  Revision 1.13  2010/02/25 18:29:45  samantha
 *  BUG FIXES FOR METHOD 2 plots:
 *  The only difference in errors in method 1 (default) plots and in method 2
 *  (reweighed sideband) plots is the additional reweighing error for method 2.
 *  In this version I have confirmed that all the plots except the njet plot) gets
 *  the same error contribution from all other errors in both method 1 and method 2.
 *  See elog#1564 for the two set of plots side-by-side.
 *  The setup is complicated when it comes to njet plot. Overall flow is better
 *  automated and the settings are dumped at the end. Still working on adding the
 *  di-pho stuff for the MET plots. Not complete yet.
 *
 *  ADDED 1. Rtypes.h is included so I do not have to use absolute numbers when
 *  refering to colors. ROOT recommends using standard ROOT color references and
 *  adding constants to get other colors. (for eg: kYellow+2)
 *  	2. GetSidebandReweighedError() to derive uncertainty of reweighing
 *  	process when using method 2. This is a cut and paste from GetQCD100Err
 *  	method where I had this error combined with the QCD ID varying error.
 *
 *  MODIFIED: 1. DrawErrorBreakDown() method to include REWEIGHT error when method 2 is
 *  used. Also I fixed the bug of exlcuding ISR/FSR etc in method 2.
 *  	2. Total error in EWK lum is stored as a vector and correctly calculated now.
 *  	3. GetQCD100Err() method is modified. Moved the reweighing error to a new
 *  function (see ADDED:2) and commented out the unused error method derived using
 *  di-jet MC.
 *  	4. The qcdjet, qcdjet_100 hists are automatically based on the choice of the
 *  sideband and sample.
 *  	5. The error of mixing QCD/PHO MC by photon fraction is taken out of the
 *  	if (QCDerrMtd), as it is common to both methods. But is not calcuated
 *  	for the NJET plot in default method.
 *  	6. When calculating errors, I have carefully removed unneccesary dependence
 *  on other hist. For eg. QCD ID error calulation needs only the correcly scaled
 *  qcd hist, so it can get the correct absolute error.
 *  	7. Modified the Ht plots bin widths to remove the first bin non-zero bin
 *  large fluctuation. This is purely due to splicing.
 *
 *  Revision 1.12  2010/02/18 17:16:54  samantha
 *  MAJOR CHANGES: I have tried to minize the mistakes when switching from
 *  case to case by introducing global variables that defines each case.
 *  All the values used for a particular case will be printed out at the end.
 *  This includes the correct background predictions which has to derived from the
 *  normalized histograms for most (except for halo and cosmic backgrounds as thoese
 *  histograms were divided by integrals before scaling to expected number. and when
 *  we call hist->Integral() it give the correct expected value.)
 *  For now there are only 3 cases
 *  1. iG30 -> pho(30) + jets(15)
 *  2. iG40 -> pho(40) + jets(15)
 *  3. iG30MET25 -> pho(30)+ jets(15) + met(25)
 *
 *  There is a option choose different sidebands too (nominal, iso added, and
 *  reweighed).
 *
 *
 *  Halo background estimates for all 3 cases is the same for now. until I
 *  reestimate it for other samples.
 *
 *  MODIFIED: MakeHistLogAndRatio() to choose different samples and sideband.
 *
 *  Revision 1.11  2010/02/14 23:04:46  samantha
 *  MAJOR CHANGES: Modified for the pho Et>40GeV and for 100% iso added reweighed
 *  sideband use for the background. See elog#1555. All final systematics are
 *  included. QCDID error method is modified for the reweighted sideband usage.
 *  still there are many places to tweak to get a correct results. The final two
 *  plots, log and ratio, are drawn on same canvas, top and bottom. Default print
 *  method is eps. Then need to converted to PDF to get the hatchting displayed
 *  corretly. Direct PDF or EPS prints does not show systematics.
 *
 *  ADDED   : 1.MakeQCDMCForSidebandReweight() to reweight photon Et of the
 *             sideband to match total backgroun, MC PHO+QCD.
 *            2. fitFunction() to derive weight function to reweight sideband.
 *  	  3. GetAlphaS_Systematic() & AlphaSsystFunc() to derive alpha_s
 *  	  systematic on the fly.
 *  	  4. DrawErrorBreakDown() method which plot the break of down of
 *  	  all the errors in absolute scale and as a ratio of the total.
 *  	  5. ZeroOutNegativeBins(), a utility to get rid of nasty situations
 *  	  that comes with hist bins getting NAN values. Given a hist, this will
 *  	  set such bins' value to 0. Usage is commented out for now for some
 *  	  reason I forot.
 *  	  5. MakeHistLogAndRatio(), two more overload to quikly make the plots
 *  	  by looping over.
 *  DELETED : 1.get_error_function() which is no longer used. I am doing real
 *             systematic calclualation now. no need for fake ones :) .
 *  MODIFIED: 1.MakeTruePhoFracStudy(), final hist pad is made not-eidtable.
 *            2.DebugMakeHist() final hist is drawn with option "P" (points).
 *  	  3.MakeRatioPlot(), the hist name were updated,eg: (which ==
 *  	  JetsInvMass) -> which == "InvMass_j1j2) etc..
 *  	  4. GetQCD100Err(): tried stick to 80 column text width. Modified to
 *  	  accomodate for the reweighted sideband errors. Also there is a option
 *  	  to choose errors based on the di-jet MC studies which proved to give
 *  	  higher error at high et, espcially for the Ht plot. But it gives very
 *  	  small errors at low et. So I am using the QCD ID vary method for now.
 *  	  When using rewegihted sideband, the errors from reweighing process and
 *  	  QCD ID varying method is combined to one and presented. WHEN USING
 *  	  REWEIGHED SIDEBAND, MUST REMEBER TO USE THE NOMINAL SIDEBAND TO DERIVE
 *  	  THE QCD ID VARY ERRORS.!
 *  	  5. main MakeHistLogAndRatio() method has lot of stuff to streamline
 *  	  the background (ewk) estimation. I am trying tranfer most of the info
 *  	  from the log book to here. and subtract the ewk/halo/cosmic from the
 *  	  sideband for the final plots. Still commented out. Need more testing
 *  	  before putting these in.
 *  	  6. main MakeHistLogAndRatio() the PHO MC systematic errors (alpha_s
 *  	  errors etc ) are evaluated only for the MC+QCD combined method.
 *  	  7. The splitting of the data reaminder between QCD and MC PHO is done
 *  	  correctly now, after subtracting all other backgrounds from the data.
 *  	  so far it did not matter that much as QCD/MC are the largest.
 *  	  8. 2nd MakeHistLogAndRatio is modified slightly with different bin
 *  	  sizes for low stats and titles.
 *
 *  Revision 1.10  2009/12/15 23:04:17  samantha
 *  ADDED:	1. Started using ../RootTools/CommonTools.hh and its utility functions.
 *  	2. Both ratio and log plot are drawn on 1 canvas using TPads. They are
 *  	to share a somewhat common X-axis.
 *  	3. NULL check for each histogram read from files.
 *  MODIFIED:1. DebugTwoHistRatio() assertion print out
 *  	2. DumpHistBin() printing header is optional and can increase the
 *  	precision of the dump.
 *  	3. MakeRatioHist(), ratio hist min/max is -1,1
 *  	4. Commented out some of the styles as I made them default by putting
 *  	those settings in the rootlogon.C
 *  	5. hist_err fill color from red to black and grid color to brown.
 *  	6. GetQCD100Err() is being modified to choose between QCDID varied
 *  	systematic or DI-JET mc based systematic. But not complete yet and hence
 *  	the new addition is still commented out.
 *  	7. Error break down dump is modified a little to add more info.
 *  	8. Dumps of hists added to THStack in both QCD/MC methods. Since I
 *  	subtract off the EWK from QCD template, some bins get <0 values which is
 *  	not refelected in the THStack. Actaully this is a bug on my part. There
 *  	should not be any bin with <0 content! This error is clear evident in
 *  	Met plot bin 11 where the total background bin content adds up to 0!
 *  	I will try to fix this in the next version.
 *  	9. MakeRatioPlot() is moved to the bottom and drawn after the log plot
 *  	is done.
 *  	10. I have few lines that are commented which I used to get the
 *  	reweighed photon sideband to see if we can fix the Ht plot.
 *  	See Elog#https://hep.baylor.edu/hep/samantha/1509
 *
 *  Revision 1.9  2009/12/01 17:57:36  samantha
 *  DELETED: 1. Unused fake_pho_frac_d
 *  	 2. Obsolete get_fit_function()
 *  	 3. Deleted old systematic error band function in MakeRatioPlot()
 *  	 4. Deleted commented out Sumw2() statements. It is already called
 *  	 within the MakeVariableBins().
 *  	 5. Deleted debugging info dumps.
 *  ADDED  : 1. DebugLogAndRatioPlots() for debugging.
 *  	 2. ISR/FSR/Q/PDF/Alpha_s unceratanties.
 *  MODIFIED: 1. MakeRatioPlot() to get errors from hist_err not from vERRROS vector
 *  as I am setting hist_err errors from the vERRORS just before this routine is
 *  called.
 *  	2. MakeVariableBins() is modified by removing the unused use_errors
 *  	parameter and hence all calls are modified accordingly.
 *  	3. In debug mode GetQCD100Err() will draw the individual error (ratio)
 *  	plots used to derive the QCD ID errors.
 *  	4. A paramater named, brname is added to MakeHistLogAndRatio(). This
 *  	identifies which fit function to be used for ISR/FSR/Q2/PDF/Alpha_s for
 *  	each plot.
 *  	5. Calling for MakeTruePhoFracStudy() is commented out.
 *
 *  Revision 1.8  2009/11/16 22:48:26  samantha
 *  ADDED: 1. old CVS log manually. New ones will be automatically added.
 *  	2. MakeTruePhoFracStudy() to check the photon purity agrees with what
 *  	is seen in inclusive photon cross section measurement.
 *  	See elog# https://hep.baylor.edu/hep/samantha/1464
 *  	3. Info to make a comparison of scales of all errors.
 *  	see https://hep04.baylor.edu/hep/samantha/1301
 *  	4. Deleted and changed some debug info dumps.
 *
 *
 * ****************************************************************************
 *	//Inserting the previous CVS logs manually here
 * //-------------------------------------------------------------------------
 * ----------------------------
 *  revision 1.7
 *  date: 2009/09/14 15:39:46;  author: samantha;  state: Exp;  lines: +63 -10
 *  still debugging
 *  ----------------------------
 *  revision 1.6
 *  date: 2009/09/10 14:57:19;  author: samantha;  state: Exp;  lines: +1031 -410
 *  DEBUGGING: I am doing an extensive debugging. So far JES/ STAT/FAKE PHOTON
 *  FRACTION have been checkeout out fine. I have worked with Sasha and he agrees
 *  that although my systamtic error seems to be unusually small, it is true.
 *  See elog:1292,1293,1294. Log book#2-pp.188 and forward. There are lot of debug
 *  functions and prints. Will clean up later. Now I am going to include the
 *  systematic error due to shape difference in sideband(background) and photon
 *  MC (signal). We did this by varying (tightning) the loose ID cuts one at a time.
 *  This was done only for the case where we used 100% sideband, which is the NJET
 *  distribution. But we need to do this for other scenario(sideband 30%, Pho MC 70%)
 *  too.
 *  ----------------------------
 *  revision 1.5
 *  date: 2009/06/30 12:31:32;  author: samantha;  state: Exp;  lines: +16 -9
 *  minor debugging prints/comments.
 *  ----------------------------
 *  revision 1.4
 *  date: 2008/08/24 17:15:10;  author: samantha;  state: Exp;  lines: +295 -53
 *  added a function to get the error function with inflated systemtics in 
 *  preparation for ICHEP08. Many more tweakings add legends/titles/labels etc. 
 *  More plots too. I still need to compare this to the version prior to 
 *  preblessing for ICHEP to figure why the systematics became so small.
 *  ----------------------------
 *  revision 1.3
 *  date: 2008/07/23 19:16:00;  author: samantha;  state: Exp;  lines: +412 -52
 *  this is version is working well and made all the plots for blessing for 
 *  ICHEP. Still I don't have agood answer to why the systematics went down 
 *  from preblessing.!
 *  ----------------------------
 *  revision 1.2
 *  date: 2008/07/23 10:16:02;  author: samantha;  state: Exp;  lines: +480 -18
 *   trying finish. errors in MakeRatioPlot routing. to be fixed!
 *   ----------------------------
 *   revision 1.1
 *   date: 2008/07/23 06:55:21;  author: samantha;  state: Exp;
 *   I am making new module to make both log and ratio plots in one.
 *   =============================================================================
 *
 */
/*}}}*/
 ...
 * MakeHistLogAndRatio() method soon after rebinning and
...
void MakeRatioPlot(const std::string which,  const int jets, const TH1* phojet,
						...
						std::vector<float> Errors, TPad *pad=0, bool debug=false)
{
							...
							bool debug=false)
{
	...
	std::cout << blue << __FUNCTION__ << "::abs path = " << abspath << std::endl;
	...
	{
		std::cout << red << __FUNCTION__ << ": no reweight error function for the " 
			<< WhichSample(iSample) << " sample! please check " << clearatt << std::endl;
		assert (false);
	}
...
*/
		size_t i = abspath.find_first_of("/");
		std::string str = abspath.substr(i, abspath.length());
		str += "/";
		str += name.c_str();
		std::cout << " path = " << str << std::endl;
		
		TFile f("PhoJets_data.root");
		std::string hademstr("Hist/SIDEBANDSYST_HADEM");
		std::string isostr("Hist/SIDEBANDSYST_ISO");
		std::string trkptstr("Hist/SIDEBANDSYST_TRKPT");
		std::string trkisostr("Hist/SIDEBANDSYST_TRKISO");
		hademstr += str;
		isostr += str;
		trkptstr += str;
		trkisostr += str;
		std::cout << " path = " << hademstr << std::endl;
		
		TH1F* hademhist = (TH1F*) f.Get(hademstr.c_str());
		TH1F* isohist = (TH1F*) f.Get(isostr.c_str());
		TH1F* trkpthist = (TH1F*) f.Get(trkptstr.c_str());
		TH1F* trkisohist = (TH1F*) f.Get(trkisostr.c_str());
		hademhist->Print();
		
		assert ( hademhist != NULL && "GetQCD100Err:: hademhist is null");
		assert ( isohist != NULL && "GetQCD100Err:: isohist is null");
		assert ( trkisohist != NULL && "GetQCD100Err:: trkisohist is null");
		assert ( trkpthist != NULL && "GetQCD100Err:: trkpthist is null");
  ...
  std::string sMcDownDir   ("Hist/EMJESDOWN/"+ path);
	...
	} else if (iSideband == iISOADDEDREWGT)
	{
 		...
 		if (iSample == iG30)
		{
			...
			std::cout << red << __LINE__ << "THIS QCD REWEIGHED FILE IS NOT CHECKED!" << std::endl;
			*/
		}
		...
		if (gDirectory->cd(sQcdDir.c_str()) == kTRUE)
		{
			...
			std::cout << red << " rewgt hist dir not found!" << std::endl;
			std::cout << "QCD dir = " << sQcdDir << clearatt << std::endl;
			ftemp->ls();
	...
	{
		std::cout << red << __FUNCTION__ << ":" << __LINE__ << "UNKNOWN sideband selection!" << clearatt << std::endl;
		assert (false);
	}
	...
	cout << red << __LINE__ << ": after rebin " << endl;
	phojet->Print();
	qcdjet->Print();
	halojet->Print();
	cosmicjet->Print();
...
/************** FAKE PHOTON FRACTION ************************************/
  ...
  //******************* NORMALIZING *************************************
	...
	const float dataLum=5306.11;	//pb-1
...
*/	
	...
	std::cout << red << "leftover = " << leftover << std::endl;
	...
	std::cout << __LINE__ << "BFORE SCALING qcdjet     = " << qcdjet->Integral("width") << std::endl;
	std::cout << __LINE__ << "BFORE SCALING qcdjet_100 = " << qcdjet_100->Integral("width") << std::endl;
	...
	std::cout << __LINE__ << "AFTER SCALING qcdjet_100 = " << qcdjet_100->Integral("width") << std::endl;
	//JES PHO MC
  ...
  const float wtnLumErr = wtnnorm * fact;
	...
	if ( ! (iSideband == iNOMINAL && QCDerrMtd == 1 && which == "NJet"))
	{
		...
		std::cout << __FUNCTION__ << "qcd/mc nominal = " << hist_qcdcopy->Integral() << ", " << mcphojet->Integral() << std::endl;
		...
		std::cout << __FUNCTION__ << "qcd/mc psigma = " << hist_qcdVaried->Integral() << ", " << hist_mcphoVaried->Integral() << std::endl;
		//nominal
	...
	if (QCDerrMtd == 1)		//_______________________ plots that use 100% qcd
  	{
		hist_qcd100Syst = (TH1F*) qcdjet_100->Clone("hist_qcd100");	//this is for default method, njet case
	...
	std::cout << red <<setw(4) << "bin"<< setw(8) << "JES"
  ...
  if (QCDerrMtd == 1) {//plots that use 100% qcd
    ...
    hs->Add(qcdjet_100);
	 ...
	 std::cout << cyan << __FUNCTION__ << ":" << __LINE__ << ": QCD100 METHOD" << std::endl;
...
*/	
  ...
  c1->cd();
  pRAT->Draw();
  MakeRatioPlot(which, jets, phojet,hist_err, title, ERRORS, pRAT, false);
	c1->cd();
...
/*	std::cout << magenta << "hist_cosmic" << std::endl;
	...
	std::cout << "my sum = " << sum << clearatt << std::endl;
*/	
...
void MakeHistLogAndRatio (int jets, std::string which, int icase=0)
//void MakeHistLogAndRatio (int jets, std::string which)
{
  TVirtualPad *pad_old = gPad;	
	...
	if (jets == 1)
	{
		...
		//else if (which == "InvMass_pj1") { xmin=0 ; xpoint1 = 500 ; xpoint2 = 600; xpoint3 = 700;xpoint4 = 1000; width1= 5 ; width2= 5 ; width3= 5 ; width4= 5 ; iREBIN =5;}
	...
	if (jets == 2) {
		...
		*/
		else if (which == "InvMass_pj1")  	{ xmin=0; xpoint1= 200; xpoint2= 300; xpoint3= 600; xpoint4= 1100; width1=20 ; width2=50 ; width3=100 ; width4=400 ; iREBIN =5;}
	...
	if (jets == 1) {
		if (which == "Et_pho") 				MakeHistLogAndRatio (which, jets, "EtCorr","E_{T}^{#gamma} (GeV)","1Jet/Photon", 											qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4, "pj1_Et_pho", 	iSample, iSideband);
		else if (which == "InvMass_pj1") MakeHistLogAndRatio (which, jets, "InvMass","Invariant Mass (#gamma,Lead Jet) (GeV/c^{2})","1Jet/PhotonLeadJet", 	qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4, "pj1_InvM_pj1",iSample, iSideband);
		else if (which == "Ht") 			MakeHistLogAndRatio (which, jets, "Ht","H_{T} (GeV)","1Jet/Event",																qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4, "pj1_Ht", 		iSample, iSideband);
		else if (which == "Et_j1") 		MakeHistLogAndRatio (which, jets, "EtCorr","E_{T}^{Lead Jet} (GeV)","1Jet/LeadJet", 										qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4, "pj1_Et_j1", 	iSample, iSideband);
    	else if (which == "NJet") 			MakeHistLogAndRatio (which, jets, "NJet15","Jet Multiplicity (E_{T}>15GeV)", "1Jet/Event",								qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4, "", iSample, iSideband);
    	else if (which == "Met") 			MakeHistLogAndRatio (which, jets, "Met","#slash{E}_{T} (GeV/c^{2})", "1Jet/Event",																		qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4, "", iSample, iSideband);
    	else if (which == "DelR") 			MakeHistLogAndRatio (which, jets, "DelR","#Delta R (#gamma, lead jet)", "1Jet/PhotonLeadJet",							qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4, "", iSample, iSideband);
    	else if (which == "DelEta") 		MakeHistLogAndRatio (which, jets, "DelEta","#Delta #eta (#gamma, lead jet)", "1Jet/PhotonLeadJet",		 				qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4, "", iSample, iSideband);
    	else if (which == "DelPhi") 		MakeHistLogAndRatio (which, jets, "DelPhi","#Delta #phi (#gamma, lead jet)", "1Jet/PhotonLeadJet", 					qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4, "", iSample, iSideband);
    	else if (which == "DetEta_pho") 	MakeHistLogAndRatio (which, jets, "DetEta","#gamma detector #eta", "1Jet/Photon",                  					qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4, "", iSample, iSideband);
    	else if (which == "DetPhi_pho") 	MakeHistLogAndRatio (which, jets, "DetPhi","#gamma detector #phi", "1Jet/Photon",											qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4, "", iSample, iSideband);
    	else if (which == "DetEta_j1") 	MakeHistLogAndRatio (which, jets, "DetEta","Lead jet detector #eta", "1Jet/LeadJet",										qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4, "", iSample, iSideband);
    	else if (which == "DetPhi_j1") 	MakeHistLogAndRatio (which, jets, "DetPhi","Lead jet detector #phi", "1Jet/LeadJet",										qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4, "", iSample, iSideband);
    	else if (which == "EvtPhi_j1") 	MakeHistLogAndRatio (which, jets, "EvtPhi","Lead jet 4-vec #phi", "1Jet/LeadJet",											qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4, "", iSample, iSideband);
	}

	if (jets == 2) {
		if (which == "Et_pho") 					MakeHistLogAndRatio (which, jets, "EtCorr","E_{T}^{#gamma} (GeV)","2Jet/Photon", 												qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4,"pj2_Et_pho", 		iSample, iSideband);
		else if (which == "InvMass_pj1") 	MakeHistLogAndRatio (which, jets, "InvMass","Invariant Mass (#gamma,Lead Jet) (GeV/c^{2})","2Jet/PhotonLeadJet", 		qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4,"pj2_InvM_pj1", 	iSample, iSideband);
		else if (which == "InvMass_pj1j2") 	MakeHistLogAndRatio (which, jets, "InvMass","Invariant Mass (#gamma, Two Lead Jets) (GeV/c^{2})","2Jet/Photon2Jets", qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4,"pj2_InvM_pj1j2", 	iSample, iSideband);
		else if (which == "Ht") 				MakeHistLogAndRatio (which, jets, "Ht","H_{T} (GeV)","2Jet/Event", 																	qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4,"pj1_Ht", 			iSample, iSideband);
		else if (which == "InvMass_j1j2") 	MakeHistLogAndRatio (which, jets, "InvMass","Invariant Mass (Two Lead Jets) (GeV/c^{2})","2Jet/2Jets", 					qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4,"pj2_InvM_j1j2", 	iSample, iSideband);
		else if (which == "Et_j1") 			MakeHistLogAndRatio (which, jets, "EtCorr","E_{T}^{Lead Jet} (GeV)","1Jet/LeadJet", 											qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4,"pj2_Et_j1", 		iSample, iSideband);
		else if (which == "SecondLeadJetEt")MakeHistLogAndRatio (which, jets, "EtCorr","E_{T}^{Second Lead Jet} (GeV)","2Jet/SecondLeadJet", 							qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4,"", iSample, iSideband);
    	else if (which == "Met") 				MakeHistLogAndRatio (which, jets, "Met","#slash{E}_{T} (GeV/c^{2})", "2Jet/Event",																			qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4,"", iSample, iSideband);
    	else if (which == "DelR") 				MakeHistLogAndRatio (which, jets, "DelR","#Delta R (#gamma, lead jet)", "2Jet/PhotonLeadJet",								qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4,"", iSample, iSideband);
    	else if (which == "DelEta") 			MakeHistLogAndRatio (which, jets, "DelEta","#Delta #eta (#gamma, lead jet)", "2Jet/PhotonLeadJet",							qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4,"", iSample, iSideband);
    	else if (which == "DelPhi") 			MakeHistLogAndRatio (which, jets, "DelPhi","#Delta #phi (#gamma, lead jet)", "2Jet/PhotonLeadJet",							qcdMcMix, xmin, xpoint1, xpoint2, xpoint3, xpoint4, width1, width2, width3, width4,"", iSample, iSideband);
	}
...
void MakeHistLogAndRatio (const int i)
{
	//good plots that works
	if (i == 1) MakeHistLogAndRatio (1, "Et_pho");
	if (i == 2) MakeHistLogAndRatio (1, "Et_j1");
	if (i == 3) MakeHistLogAndRatio (1, "InvMass_pj1");
	if (i == 4) MakeHistLogAndRatio (1, "Ht");
	if (i == 5) MakeHistLogAndRatio (2, "Et_pho");
	if (i == 6) MakeHistLogAndRatio (2, "Et_j1");
	if (i == 7) MakeHistLogAndRatio (2, "InvMass_pj1");
	if (i == 8) MakeHistLogAndRatio (2, "InvMass_pj1j2");
	if (i == 9) MakeHistLogAndRatio (2, "InvMass_j1j2");
	
	if (i == 10) MakeHistLogAndRatio (1, "NJet");

	if (i== 11) MakeHistLogAndRatio (2, "Ht");
	
	if (i== 12) MakeHistLogAndRatio (1, "Met");
	if (i== 13) MakeHistLogAndRatio (1, "DetEta_pho");
	if (i== 14) MakeHistLogAndRatio (1, "DetEta_j1");
	if (i== 15) MakeHistLogAndRatio (1, "DetR_pj1");
...
{
	for (int i=1;i<=15;++i) MakeHistLogAndRatio (i);
}



//list current temporary edit
//1. modified for Ht plot. sideband is from weighted sideband root file - Dec 14,2009
//	---commented out - Dec 15, 2009
//2. Need to figure out the exact point I should call ZeroOutNegativeBins(). Depends on this,
//my QCD ID error can be zerod for most bins. clear a problem for Met plot!
//01-22-2010 - modified for reweighing sideband for photon et>40
