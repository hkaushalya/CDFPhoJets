...
void MakeHist_Exlcusive (int jets, const std::string& name, const std::string& title,
				 float lolimit, float hilimit, int rebin, const std::string& path, const int qcd_opt)
{
	std::string phofile("PhotonJets_Exclusive1_2Jet.root");
	std::string mcphofile("MCPhotonJets_Exclusive1_2Jet.root");
	std::string zeefile("zeeMCPhotonJets_Exclusive1_2Jet.root");
	std::string zmmfile("zmmMCPhotonJets_Exclusive1_2Jet.root");
	std::string zttfile("zttMCPhotonJets_Exclusive1_2Jet.root");
	std::string wenfile("wenMCPhotonJets_Exclusive1_2Jet.root");
	std::string wmnfile("wmnMCPhotonJets_Exclusive1_2Jet.root");
	std::string wtnfile("wtnMCPhotonJets_Exclusive1_2Jet.root");
	std::string halofile("HaloJets_Exclusive1_2Jet.root");
	std::string cosmicfile("CosmicJets_Exclusive1_2Jet.root");
	std::string qcdfile("QCDJets_Exclusive1_2Jet.root");
...
void MakeHist_Exlcusive (int jets,std::string name,  std::string opt = "")
{
  ...
  const bool opt_qcddown = (opt.find ("QCDDOWN") != std::string::npos);
	...
	if (jets == 1) {
	  if (name == "InvMass") 	MakeHist_Exlcusive (jets, "InvMass","Invariant Mass(#gamma,Lead Jet)",0,1000,5,"1Jet/PhotonLeadJet", qcd_scale);
	  else if (name == "pet") 	MakeHist_Exlcusive (jets, "EtCorr","E_{T}^{#gamma}",0,400,5,"1Jet/Photon", qcd_scale);
	  else if (name == "met")	MakeHist_Exlcusive (jets, "Met","MET",0,200,1,"1Jet/Event", qcd_scale);
	  else if (name == "njet") 	MakeHist_Exlcusive (jets, "NJet15","NJet15",0,15,1,"1Jet/Event", qcd_scale);
	  else if (name == "ht")  	MakeHist_Exlcusive (jets, "Ht","H_{T}",0,800,2,"1Jet/Event", qcd_scale);
	  else if (name == "jet") 	MakeHist_Exlcusive (jets, "EtCorr","E_{T}^{lead Jet}",0,400,5,"1Jet/LeadJet", qcd_scale);
	  else if (name == "etratio") MakeHist_Exlcusive (jets, "EtRatio","E_{T} ratio (lead Jet/ #gamma)",0,10,2,"1Jet/PhotonLeadJet", qcd_scale);
	  else if (name == "delphi") 	MakeHist_Exlcusive (jets, "DelPhi","#Delta #phi^{#gamma,lead jet}",0,5,1,"1Jet/PhotonLeadJet", qcd_scale);
	  else if (name == "delr") MakeHist_Exlcusive (jets, "DelR", "#Delta R^{#gamma, lead Jet}",0,7,1,"1Jet/PhotonLeadJet", qcd_scale);
	}
	if (jets == 2) {
	  if (name == "InvMass") 	MakeHist_Exlcusive (jets, "InvMass","Invariant Mass(#gamma,Two Lead Jets)",0,1000,5,"2Jet/Photon2Jets", qcd_scale);
	  else if (name == "Met")	MakeHist_Exlcusive (jets, "Met","MET",0,200,1,"2Jet/Event", qcd_scale);
	  if (name == "jetsInvMass") 	MakeHist_Exlcusive (jets, "InvMass","Invariant Mass(Lead two Lead Jets)",0,1000,5,"2Jet/2Jets", qcd_scale);
	  else if (name == "NJet15") 	MakeHist_Exlcusive (jets, "NJet15","NJet15",0,15,1,"2Jet/Event", qcd_scale);
	  else if (name == "PhoEt") 	MakeHist_Exlcusive (jets, "EtCorr","E_{T}^{#gamma}",0,400,5,"2Jet/Photon", qcd_scale);
	  else if (name == "pj1DelPhi") 	MakeHist_Exlcusive (jets, "DelPhi","#Delta #phi^{#gamma,lead jet}",0,4,1,"2Jet/PhotonLeadJet", qcd_scale);
	  else if (name == "pj2DelPhi") 	MakeHist_Exlcusive (jets, "DelPhi","#Delta #phi^{#gamma,2nd lead jet}",0,4,1,"2Jet/Photon2ndLeadJet", qcd_scale);
	  else if (name == "j1j2DelPhi") 	MakeHist_Exlcusive (jets, "DelPhi","#Delta #phi^{lead jet,2nd lead jet}",0,4,1,"2Jet/2Jets", qcd_scale);
	  else if (name == "Ht")  	MakeHist_Exlcusive (jets, "Ht","H_{T}",0,800,2,"2Jet/Event", qcd_scale);
	  else if (name == "j1Et") 	MakeHist_Exlcusive (jets, "EtCorr","E_{T}^{lead Jet}",0,400,5,"2Jet/LeadJet", qcd_scale);
	  else if (name == "j2Et") 	MakeHist_Exlcusive (jets, "EtCorr","E_{T}^{2nd lead Jet}",0,400,5,"2Jet/SecondLeadJet", qcd_scale);
	  else if (name == "pj1Etratio") MakeHist_Exlcusive (jets, "EtRatio","E_{T} ratio (lead Jet/ #gamma)",0,10,1,"2Jet/PhotonLeadJet", qcd_scale);
	  else if (name == "pj2Etratio") MakeHist_Exlcusive (jets, "EtRatio","E_{T} ratio (2nd lead Jet/ #gamma)",0,10,1,"2Jet/Photon2ndLeadJet", qcd_scale);
	  else if (name == "j1j2Etratio") MakeHist_Exlcusive (jets, "EtRatio","E_{T} ratio (2nd lead Jet/lead Jet)",0,2.5,3,"2Jet/2Jets", qcd_scale);
	}
...
void MakeHist_Exlcusive ()
{
	MakeHist_Exlcusive (1, "InvMass");
	MakeHist_Exlcusive (1, "pet");
	MakeHist_Exlcusive (1, "met");
	MakeHist_Exlcusive (1, "njet");
	MakeHist_Exlcusive (1, "ht");
	MakeHist_Exlcusive (1, "jet");
	MakeHist_Exlcusive (1, "etratio");
	//MakeHist_Exlcusive (1, "delphi");
	MakeHist_Exlcusive (1, "delr");
	MakeHist_Exlcusive (2, "InvMass");
	MakeHist_Exlcusive (2, "Met");
	MakeHist_Exlcusive (2, "jetsInvMass");
	MakeHist_Exlcusive (2, "NJet15");
	MakeHist_Exlcusive (2, "PhoEt");
	//MakeHist_Exlcusive (2, "pj1DelPhi");
	//MakeHist_Exlcusive (2, "pj2DelPhi");
	//MakeHist_Exlcusive (2, "j1j2DelPhi");
	MakeHist_Exlcusive (2, "Ht");
	MakeHist_Exlcusive (2, "j1Et");
	MakeHist_Exlcusive (2, "j2Et");
	MakeHist_Exlcusive (2, "pj1Etratio");
	MakeHist_Exlcusive (2, "pj2Etratio");
	MakeHist_Exlcusive (2, "j1j2Etratio");
};
