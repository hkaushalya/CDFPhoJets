...
//-----------------------------------------------------------------------------
  ...
  // Get ride of the profile histograms
			       ...
			       int         FilteringMode) 
{
  ...
  if (! cl) {
    ...
    else if (ModuleName) m->SetTitle(ModuleName);
...
//-----------------------------------------------------------------------------
  ...
  fHeaderBlock->GetEvent()->SetEventNumber(rn,ev,fHeaderBlock->SectionNumber());
  passed = 1;
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
  if (( rn < fMinRunNumber) || (rn > fMaxRunNumber )) {
                                                            return -2;
  }
...
//-----------------------------------------------------------------------------
// each module is supposed to talk to tree and to request the data (branches) 
// it needs
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
  ...
  if (fPrintLevel == 0) {
    if (fNProcessedEvents % fNEventsToReport == 0) {
  		...
  		// a hack to get percentage of the job finished -sam -Oct 21,2009
      //fHeaderBlock->Print(Form(" >>> TStnAna: Processed %10i events",
		//	       fNProcessedEvents));
		 std::stringstream mystr;
		 int perc = (int) (fNProcessedEvents/dEntries2bProcessed * 100.);
		 mystr << " >>> TStnAna: Processed "<< std::setw(10) << fNProcessedEvents 
			 << " events out of " << std::setw(10) << dEntries2bProcessed
			 << std::setw(2) << "[" << std::setw(4) << perc << "%]";
		 fHeaderBlock->Print(Form(mystr.str().c_str()));
	 }
  }
