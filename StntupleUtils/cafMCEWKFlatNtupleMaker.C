...
void cafMCEWKFlatNtupleMaker(int ind, int MomPdg, int partype, int syscode)
{
	...
	if (partype == 0 || partype == 1) split = 500;
	if (partype == 2) split = 50;	
	
		std::cout << "split, IND=" << ind << ", " << split <<std::endl;
		ap->SetSplit(ind, split);
		std::cout << "syscode, MomPdg, parType=" << syscode << ", " << MomPdg << ", " << partype <<std::endl;
	...
	std::string skimfile = base_dir + "EleJetNoMatchAndJetEtaProblem_MCPho.root_"+ str_ind;
