...
void runPho2Jets_caf(const int nEvts = 100, const int dataset = 10,
							...
							const int NcafSections=20) 
{
	...
	TStnDataset *dsp[Ndatasets];
		...
		{
			assert(false && "no valid dataset defined!");
	...
	trigMod->SetNeedTriggers(1);				// 1= require tigger, 0=NO (for MC)
	trigMod->SetNeedMinVtx(1);
	trigMod->SetNeedVtxZcut(1);				//1= require z<60 0=not cut on z
   ...
   jf->SetDebug(0);
	...
	EventProperties * evtProp = new EventProperties;                                                                                                       
		evtProp->SetSummaryStat(bSummaryStat);    //enable/disable en
	...
	PhoJets *phoSel = new PhoJets();
		...
		phoSel->SetJetMaxDetEta(3.0);
		phoSel->SetJetMinEt(15.0);
	...
	file << "PhoJets_" << suffix <<"_" << cafind << "_" << NcafSections << ".root";
