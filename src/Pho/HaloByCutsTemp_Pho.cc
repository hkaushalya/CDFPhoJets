...
#include "samantha/Pho/HaloByCutsTemp_Pho.hh"
#include "Stntuple/loop/TStnAna.hh"
#include "Stntuple/obj/TStnPhoton.hh"
#include <iostream>

ClassImp(HaloByCutsTemp_Pho)

//_____________________________________________________________________________
HaloByCutsTemp_Pho::HaloByCutsTemp_Pho(const char* name, const char* title):
  TStnModule(name,title),
  qMc(false),
  bRunPermit(true)
{
	std::cout << "Hello I am HaloByCutsTemp_Pho module" << std::endl;
}

//_____________________________________________________________________________
HaloByCutsTemp_Pho::~HaloByCutsTemp_Pho() {
}

//_____________________________________________________________________________
void HaloByCutsTemp_Pho::SaveHistograms() {
}

//_____________________________________________________________________________
void HaloByCutsTemp_Pho::BookHistograms()
{
	DeleteHistograms();
...
//_____________________________________________________________________________
int HaloByCutsTemp_Pho::BeginJob()
{
...
//_____________________________________________________________________________
int HaloByCutsTemp_Pho::BeginRun()
{
...
/*-------------------------------------------------------------------*/
/*-------------------------------------------------------------------*/
int HaloByCutsTemp_Pho::Event(int ientry)
{
	// Event  loop
...
/*-------------------------------------------------------------------*/
void HaloByCutsTemp_Pho::Cleanup()
{
...
/*-------------------------------------------------------------------*/
void HaloByCutsTemp_Pho::FillDataBlocks(int ientry)
{
}
/*-------------------------------------------------------------------*/
/*-------------------------------------------------------------------*/
void HaloByCutsTemp_Pho::FillPhiWedgeHistogram(PhiWedgeHist_t& Hist, SuperPhoton* sp)
{
	if (!sp) {
...
/*-------------------------------------------------------------------*/
/*-------------------------------------------------------------------*/
void HaloByCutsTemp_Pho::BookPhiWedgeHistograms(PhiWedgeHist_t& Hist,
...
/*-------------------------------------------------------------------*/
/*-------------------------------------------------------------------*/
void HaloByCutsTemp_Pho::FillHistograms(EventHist_t& Hist, SuperPhoton* sp)
{
		Hist.PhoEmTime->Fill(sp->GetEmTime());
...
/*-------------------------------------------------------------------*/
/*-------------------------------------------------------------------*/
void HaloByCutsTemp_Pho::BookEvtHistograms(EventHist_t& Hist,
...
//  END JOB SUMMARY
//_____________________________________________________________________________
int HaloByCutsTemp_Pho::EndJob() {
	std::string modname = GetName();
	std::string msg;
	msg  = "[HCP:00:]----- end job: ---- " + modname + "\n";
	msg += "[HCP:01:] Events Run Over ------------ = " + ToStr(counter.evtsRunOver) + "\n";
	msg += "[HCP:02:] Events Pass this module ---- = " + ToStr(counter.evtsPassModule) + "\n";
	msg += "[HCP:03:] Halo Events Found ---------- = " + ToStr(counter.haloEvtsFound) + "\n";
	msg += "[HCP:04:] Halo+ 1 Jet Events --------- = " + ToStr(hCount.iCounter->GetBinContent(3)) + "\n";
	msg += "[HCP:05:] Halo+ 2 Jet Events --------- = " + ToStr(hCount.iCounter->GetBinContent(4)) + "\n";
	msg += "---------------------------------------------------\n";
	std::cout << msg;
	return 0;
}
