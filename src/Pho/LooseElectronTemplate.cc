///////////////////////////////////////////////////////////
// this is my loose e + jets template same as other temps//
// except select looseelectrons. this must be exactly    //
// same as ElectronTemplate except select LooseElectrons //
// inplace of TightElectrons. This is used to get errror //
// for electron background estimate.                     //
// loop over all electrons to find fake rate for each.   //
...
/* 
 * $Log: LooseElectronTemplate.cc,v $
 * Revision 1.4  2009/06/01 22:10:36  samantha
 * Minor change: from int to unsigned in for loop starter to remove compiler
 * warning.
 *
 *
 */
...
ClassImp(LooseElectronTemplate)
//_____________________________________________________________________________
LooseElectronTemplate::LooseElectronTemplate(const char* name, const char* title):
...
{
	//constructor
	std::cout << "Hello I am LooseElectronTemplate module" << std::endl;
}

//_____________________________________________________________________________
LooseElectronTemplate::~LooseElectronTemplate() {
	//destructor
}

//_____________________________________________________________________________
void LooseElectronTemplate::SaveHistograms() {
}

//_____________________________________________________________________________
void LooseElectronTemplate::BookHistograms()
{
...
//_____________________________________________________________________________
int LooseElectronTemplate::BeginJob()
{
	//begin job
	...
	tagEleMod = (TagLooseElectrons*) ((TStnAna*) GetAna()->GetModule("TagLooseElectrons"));
	if (!tagEleMod) {
		StdOut(__FILE__,__LINE__,3,"Loose Electron tagging module required!.");
		sErrs.append("Loose Electron Tagging mod not found. ");
		bRunPermit = false;
	}
	...
	std::string file1("LooseEleFakeRate_1Jet.sxt");
	...
	std::string file2("LooseEleFakeRate_2Jet.sxt");
...
//_____________________________________________________________________________
int LooseElectronTemplate::BeginRun()
{
	return 0;
} //BeginRun
...
// EVENT LOOP
/*-------------------------------------------------------------------*/
int LooseElectronTemplate::Event(int ientry)
{
	SetPassed(1);
	if (!bRunPermit) {
	...
	for (int i=0; i < NsuperPho; i++) {
		if (! initSpMod->GetSuperPhoton(i)->IsLooseElectron())  continue; // select electrons
...
/*-------------------------------------------------------------------*/
void LooseElectronTemplate::Cleanup()
{
...
/*-------------------------------------------------------------------*/
void LooseElectronTemplate::FillDataBlocks(int ientry)
{
}
...
//  END JOB SUMMARY
//_____________________________________________________________________________
int LooseElectronTemplate::EndJob() {
	
	std::string modname = GetName();
	std::string msg;
	msg  = "[LET:00:]----- end job: ---- " + modname + "\n";
	msg += "[LET:01:] Events Run Over ------------ = " + ToStr(hCount.iCounter->GetBinContent(1)) + "\n";
	msg += "[LET:02:] Events Pass this module ---- = " + ToStr(hCount.iCounter->GetBinContent(2)) + "\n";
	msg += "[LET:03:] ele + 1 Jet Events --------- = " + ToStr(hCount.iCounter->GetBinContent(3)) + "\n";
	msg += "[LET:04:] ele + 2 Jet Events --------- = " + ToStr(hCount.iCounter->GetBinContent(4)) + "\n";
	msg += "[LET:05:] EM Time window used -------- = " + ToStr(GetEmTimeWindowMin()) + "," + ToStr(GetEmTimeWindowMax())+ "\n";
